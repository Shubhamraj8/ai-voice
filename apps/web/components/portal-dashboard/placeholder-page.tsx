...
export function PortalPlaceholderPage({ href }: PortalPlaceholderPageProps) {
  const item = getPortalNavItem(href);
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="rounded-2xl border border-zerqo-line bg-white p-8 shadow-sm">
        <p className="font-mono text-xs font-medium text-zerqo-orange">Phase 5</p>
        ...
        <p className="mt-3 max-w-md text-sm leading-relaxed text-zerqo-muted">
          The client portal shell is live — auth, navigation, and your workspace profile are wired.
          {item.label} data connects when Phase 5 ships.
        </p>
        <div className="mt-6 h-px bg-zerqo-line" />
        <ul className="mt-5 space-y-2.5">
          {["Tenant-scoped access enforced", "API connected via /me", "Navigation ready"].map(
            (line) => (
